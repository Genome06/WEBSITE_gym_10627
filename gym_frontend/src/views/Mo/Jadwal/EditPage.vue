...
<script>
    ...
    export default {
        setup() {
            ...
            //get API from Laravel Backend
            axios
                .get("http://localhost:8000/api/jadwalUmum/" + id,)
                .then(response => {
                    //assign state posts with response data
                    jadwalUmum.id_kelas = response.data.data.id_kelas
                    jadwalUmum.id_instruktur = response.data.data.id_instruktur
                    jadwalUmum.waktu = response.data.data.waktu
                    jadwalUmum.waktu_hari = response.data.data.waktu_hari
                }).catch(error => {
                    console.log(error.response.data)
                })
            })
            ...
            function update() {
                ...
                axios.defaults.headers.common = {'Authorization': `Bearer ${token}`}
                axios
                    .put("http://localhost:8000/api/jadwalUmum/" + id, {
                        id_kelas : id_kelas,
